...
if st.session_state.get("username", "").lower() == "emp":
    ...
    invoice_summary["COD Total Amount (Net of Adj - 50/10)"] = f"Rs {cod_total+staff_tab_amount+complaint_amount+pr_tab_amount:,.0f}"
else:
    invoice_summary["COD Total Amount "] = f"Rs {cod_total+staff_tab_amount+complaint_amount+pr_tab_amount:,.0f}"
