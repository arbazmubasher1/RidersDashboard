...
if selected_riders or selected_invoice_type or selected_shifts:
    st.markdown(
        ...
        <div class='card'>
            ...
            <div class='card-metric'>📅 Date: {start_date.strftime('%d-%b-%Y')} to {end_date.strftime('%d-%b-%Y')}</div>
            <div class='card-metric'>🧍 Rider: {", ".join(selected_riders) if selected_riders else "None"} selected</div>
...
basic_metrics = {
    ...
    "Partial Orders": (filtered_df["Partial Order Return (Amount)"] > 0).sum()
}
